...
//完美代码：右击将玩家的营养值调整为最佳状态，并附带10秒饱和效果，20分钟冷却
ItemEvents.rightClicked("kubejs:code_perfection", event => {
    ...
    event.server.runCommandSilent(`effect give ${playerName} minecraft:saturation 200 0 false`)
    event.server.runCommandSilent(`particle minecraft:heart ${player.x} ${player.y} ${player.z} 0.5 0.5 0.5 0.01 15 normal`)
    event.player.addItemCooldown(event.item, 20*20*60)
})
